...
from smzdm.views import home_page, login, register
...
from smzdm.views import error_login_message, error_register_notvaild
...
class registerTest(TestCase):
    def test_register_url_resolves_to_register_view(self):
        client = Client()
        response = client.get('/smzdm/register/')
        self.assertEqual(response.resolver_match.func, register)

    def test_register_view_returns_correct_html(self):
        client = Client()
        response = client.get('/smzdm/register/')

        expected_html = render_to_string('register.html', {'form': UserForm()})
        patten = re.compile(r"<input type='hidden'.*?/>")
        searchObj = patten.sub('', response.content.decode())
        self.assertEqual(searchObj, expected_html)

    def test_register_view_renders_register_template(self):
        response = self.client.get('/smzdm/register/')
        self.assertTemplateUsed(response, 'register.html')

    def test_register_view_uses_UserForm(self):
        response = self.client.get('/smzdm/register/')
        # 使用 assertIsInstance 确认视图使用的是正确的表单类。
    ...
    # text
    @skip
    def test_register_to_register_and_redirect_home_page(self):
        client = Client()
        response = client.post('/smzdm/register/', {'username': 'test', 'password': 'test', 'password_again': 'test'},
                               follow=True)
        self.assertEqual(response.resolver_match.func, home_page)
    ...
    def test_register_to_register_and_redirect_home_page(self):
        client = Client()
        response = client.post('/smzdm/register/', {'username': '123', 'password': 'test', 'password_again': 'test'},
                               follow=True)
        self.assertEqual(response.resolver_match.func, home_page)
    ...
    def test_register_to_register(self):
        client = Client()
        client.post('/smzdm/register/', {'username': '123', 'password': 'test', 'password_again': 'test'}, follow=True)
        response = client.login(username='123', password='test')
        self.assertTrue(response)

    def test_for_invalid_register_of_text_username(self):
        client = Client()
        response = client.post('/smzdm/register/', {'username': 'text', 'password': 'test', 'password_again': 'test'},
                               follow=True)
        self.assertEqual(response.resolver_match.func, register)

    def test_for_invalid_register_shows_error_on_page(self):
        client = Client()
        response = client.post('/smzdm/register/', {'username': 'text', 'password': 'text', 'password_again': 'test'},
                               follow=True)

        self.assertContains(response, escape(error_register_notvaild))
    ...
    def test_for_invalid_of_password_not_euqual_password_again(self):
        client = Client()
        response = client.post('/smzdm/register/', {'username': '123', 'password': 'text', 'password_again': 'test1'},
                               follow=True)
        self.assertContains(response, escape(error_register_notvaild))
